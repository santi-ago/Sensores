...
/*******************************************************************************
* Function Name: ADC2_theACLK_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_Start(void) 
{
    /* Set the bit to enable the clock. */
    ADC2_theACLK_CLKEN |= ADC2_theACLK_CLKEN_MASK;
	ADC2_theACLK_CLKSTBY |= ADC2_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ADC2_theACLK_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    ADC2_theACLK_CLKEN &= (uint8)(~ADC2_theACLK_CLKEN_MASK);
	ADC2_theACLK_CLKSTBY &= (uint8)(~ADC2_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC2_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_StopBlock(void) 
{
    if ((ADC2_theACLK_CLKEN & ADC2_theACLK_CLKEN_MASK) != 0u)
    {
...
#if defined(ADC2_theACLK__CFG3)
        CLK_DIST_AMASK = ADC2_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC2_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC2_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC2_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC2_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC2_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ADC2_theACLK_CLKSTBY &= (uint8)(~ADC2_theACLK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ADC2_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ADC2_theACLK_GetSourceRegister();
    uint16 oldDivider = ADC2_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC2_theACLK_CLKEN & ADC2_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC2_theACLK_DIV_PTR, clkDivider);
                ADC2_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC2_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC2_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC2_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ADC2_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 ADC2_theACLK_GetDividerRegister(void) 
{
    return CY_GET_REG16(ADC2_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC2_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_SetModeRegister(uint8 modeBitMask) 
{
    ADC2_theACLK_MOD_SRC |= modeBitMask & (uint8)ADC2_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC2_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_ClearModeRegister(uint8 modeBitMask) 
{
    ADC2_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC2_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC2_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC2_theACLK_GetModeRegister(void) 
{
    return ADC2_theACLK_MOD_SRC & (uint8)(ADC2_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC2_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ADC2_theACLK_GetDividerRegister();
    uint8 oldSrc = ADC2_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC2_theACLK_MOD_SRC |= CYCLK_SSS;
        ADC2_theACLK_MOD_SRC =
            (ADC2_theACLK_MOD_SRC & (uint8)(~ADC2_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC2_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC2_theACLK_GetSourceRegister(void) 
{
    return ADC2_theACLK_MOD_SRC & ADC2_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC2_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC2_theACLK_SetPhaseRegister(uint8 clkPhase) 
{
    ADC2_theACLK_PHASE = clkPhase & ADC2_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC2_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC2_theACLK_GetPhaseRegister(void) 
{
    return ADC2_theACLK_PHASE & ADC2_theACLK_PHASE_MASK;
}
